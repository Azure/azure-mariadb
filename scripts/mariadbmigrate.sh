...
#
# PURPOSE
# Migrate Azure Database for MariaDB Server from Basic Pricing Tier to General Purpose/Memory Optimized Pricing Tier
# 
# DESCRIPTION
# Pricing tiers in Azure Database for MariaDB
...
# This script uses mysqldump to extract a MariaDB database into a dump file
# and mysql to restore the MariaDB database from an archive file created by mysqldump.
#
# Migrate your MariaDB database using dump and restore
# https://docs.microsoft.com/en-us/azure/mariadb/howto-migrate-dump-restore
#
# PREREQUISITES
...
# https://docs.microsoft.com/en-us/cli/azure/mariadb/server?view=azure-cli-latest
# 
# USAGE
# bash mariadbmigrate.sh
#         --source-subscription-id 'ffffffff-ffff-ffff-ffff-ffffffffffff'
#         --source-resource-group-name 'sourceRG'
#         --source-server-name 'sourcemariadb'
...
#         --target-server-name 'targetmariadb'
...
function usage()
{
    echo ""
    echo "PURPOSE"
    echo "Migrate Azure Database for MariaDB Server from Basic Pricing Tier to General Purpose/Memory Optimized Pricing Tier"
    echo ""
    echo "DESCRIPTION"
    echo "Pricing tiers in Azure Database for MariaDB - Single Server"
    echo "https://docs.microsoft.com/en-us/azure/mariadb/concepts-pricing-tiers"
    echo ""
    ...
    echo "This script uses mysqldump to extract a MariaDB database into a dump file"
    echo "and mysql to restore the MariaDB database from an archive file created by mysqldump."
    echo ""
    echo "Migrate your MariaDB database using dump and restore"
    echo "https://docs.microsoft.com/en-us/azure/mariadb/howto-migrate-dump-restore"
    echo ""
    echo "PREREQUISITES"
    ...
    echo "https://docs.microsoft.com/en-us/cli/azure/mariadb/server?view=azure-cli-latest"
    echo ""
    echo "USAGE"
    echo "bash mariadbmigrate.sh"
    echo -e "\t--source-subscription-id 'ffffffff-ffff-ffff-ffff-ffffffffffff'"
    echo -e "\t--source-resource-group-name 'sourceRG'"
    echo -e "\t--source-server-name 'sourcemariadb'"
    ...
    echo -e "\t--target-server-name 'targetmariadb'"
...
echo "Target Server : https://portal.azure.com/#resource/subscriptions/$TARGET_SUBSCRIPTION_ID/resourceGroups/$TARGET_RESOURCE_GROUP_NAME/providers/Microsoft.DBforMariaDB/servers/$TARGET_SERVER_NAME/overview"